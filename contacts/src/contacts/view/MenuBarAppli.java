...
public class MenuBarAppli extends MenuBarAbstract {
	...
	public void init() {
		...
		addMenuItem("Personnes", menu, e -> managerGui.showView(ViewPersonneList.class));
		
		addMenuItem("Mémo", menu, e -> managerGui.showView(ViewMemoList.class));

		addMenuItem("Catégories", menu, flagRoleAdmin, e -> managerGui.showView(ViewCategorieList.class));

		addMenuItem("Comptes ", menu, flagRoleAdmin, e -> managerGui.showView(ViewCompteCombo.class));

		addMenuItem("Service ", menu, flagRoleAdmin, e -> managerGui.showView(ViewServiceList.class));
		// Menu Tests

		menu = addMenu("Test", flagRoleAdmin);

		addMenuItem("DaoCategorie", menu, e -> managerGui.showView(ViewTestDaoCategorie.class));

		addMenuItem("DaoPersonne", menu, e -> managerGui.showView(ViewTestDaoPersonne.class));

		addMenuItem("DaoCompte", menu, e -> managerGui.showView(ViewTestDaoCompte.class));

		addMenuItem("DaoMemo", menu, e -> managerGui.showView(ViewTestDaoMemo.class));
		
		addMenuItem("DaoService", menu, e -> managerGui.showView(ViewTestDaoService.class));
		...
		flagRoleUtil.bind(Bindings.createBooleanBinding(
				() -> flagConnexion.get() && compteActif.get().isInRole(Roles.UTILISATEUR), compteActif));
		flagRoleAdmin.bind(Bindings.createBooleanBinding(
				() -> flagConnexion.get() && compteActif.get().isInRole(Roles.ADMINISTRATEUR), compteActif));
